...
pis = input("datum: ")
datum = re.sub("\D", "", pis)
d = dt.datetime.strptime(datum, '%d%m%Y').strftime('%Y-%m-%d')
print(d)
...
def regEx():
    x = input("b: ")
    z = re.sub("\s", "", c.hump(x))
    return print(z[0].lower() + z[1:])

def heslo(pocetUzivatelu):
    i = 0
    for i in range(i, pocetUzivatelu):
        upper = ''
        for _ in range(3):
            upper_int = random.randint(65, 90)
            upper += (chr(upper_int))
        lower = ''
        for _ in range(3):
            lower_int = random.randint(97, 122)
            lower += (chr(lower_int))
        number = ''
        for _ in range(3):
            number_int = random.randint(49, 57)
            number += (chr(number_int))
        znak = ''
        znak_int = random.randint(40, 47)
        znak += (chr(znak_int))
        print(upper + lower + znak + number)
    return
